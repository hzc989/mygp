...
<script type = "text/javascript" language = "javascript">
  function delete_alarm(obj)
  {
   if(confirm("确定删除该举报?"))
    	{
    		location.href="../deal/alarm_delete.php?item_alarm_id="+obj;
    	}else{
    		return false;
    	}
  }
...
<input type="text" name="owner" placeholder="搜索原帖所属人" />&nbsp;&nbsp;
...
$owner=$_GET['owner'];
if($status!=""){
	$sql="select * from item_alarm where status=".$status." and owner like '%".$owner."%'";
}else{
	$sql="select * from item_alarm where owner like '%".$owner."%'";
}
...
<td><!--输出违规详情-->
<a class="btn" data-toggle="modal" href="#ItemModal<?php  echo $row['id']; ?>">查看</a>
<div class="modal hide" id="ItemModal<?php  echo $row['id']; ?>">
	...
	<?php 
	$reason_res=mysql_query("select * from item_alarm where id=".$row["id"]);//执行sql语句，得到一个结果集
...
<td><!--审查操作(below)-->
<a href="item_alarm_modify.php?id=<?php echo $row['id']; ?>" class="btn btn-info">审查</a><!--新建一页进行修改-->&nbsp;&nbsp;
<?php if($row["status"]=='0'){ ?>
...
if ($page != 1) { //页数不等于1
?>
<a href="item_alarm_mag.php?page=1&status=<?php echo $status; ?>&owner=<?php echo $owner; ?>">首页</a><!--回到首页-->
<a href="item_alarm_mag.php?page=<?php echo $page - 1;?>&status=<?php echo $status; ?>&owner=<?php echo $owner; ?>">上一页</a> <!--显示上一页-->
<?php
}
?>
<?php echo $page."&nbsp;"; ?>
...
?>
<a href="item_alarm_mag.php?page=<?php echo $page + 1;?>&status=<?php echo $status; ?>&owner=<?php echo $owner; ?>">下一页</a>
<a href="item_alarm_mag.php?page=<?php echo $totalPage;?>&status=<?php echo $status; ?>&owner=<?php echo $owner; ?>">尾页</a><!--去尾页-->
</td></tr><!--此tr为页码的tr-->
<?php
  }//页码end
?>
