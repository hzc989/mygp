...
$count=mysql_query("select count(*) from item where status='0' and title like '%".$_GET['title']."%'"); //获得记录总数
...
$result=mysql_query("select * from item where status='0' and title like '%".$_GET['title']."%'"." order by last_modify desc limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
...
if ($page != 1) { //页数不等于1
?>
<a href="itemsearch.php?page=<?php echo $page - 1;?>&title=<?php echo $_GET['title'];?>&search=">上一页</a> <!--显示上一页-->
<?php
}
?>
<?php echo $page; ?>
...
?>
<a href="itemsearch.php?page=<?php echo $page + 1;?>&title=<?php echo $_GET['title'];?>&search=">下一页</a>
<?php
} 
?>
</td>
